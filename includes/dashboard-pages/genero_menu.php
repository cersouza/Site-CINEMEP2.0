<?php 
	...
	if ($pagina > 1)
	{
		...
		{
			$pag .= "<li>
			<a class='btn btn-dark' href='dashboard.php?tb=genero&op=menu&s=" . ($inicio - $exiba) .
			"&p=" . $pagina . 
		...
		for ($i = 1; $i <= $pagina; $i++)
		{
			if ($i != $pagina_correta)
			{
				$pag .= "<li><a class='btn btn-dark' href='dashboard.php?tb=genero&op=menu&s="
				. ($exiba * ($i - 1)) . "&p=" .
		...
		{
			$pag .= "<li>
			<a class='btn btn-dark' href='dashboard.php?tb=genero&op=menu&s="
			. ($inicio + $exiba) .
			"&p=" . $pagina . "&ordem=" . $ordem . "'>Próximo</a></li>";
		}
		else
		{
			$pag .= "<li class='btn btn-dark' disabled><a>Próximo</a></li>";
		}
	}
