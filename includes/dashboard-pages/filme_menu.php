<?php 	
	...
	if (mysqli_num_rows($r) > 0)
	{
		...
		while ($row = mysqli_fetch_array($r, MYSQLI_ASSOC))
		{
            ...
            <a href='dashboard.php?tb=atorfilme&op=menu&fil=" . $row['fil_codigo'] ."' class='btn btn-xs btn-primary'>Ator</a>
			</td></tr>";		
		}
	...
	if ($pagina > 1)
	{
		...
		{
			$pag .= "<li>
			<a class='btn btn-dark' href='dashboard.php?tb=filme&op=menu&s=" . ($inicio - $exiba) .
			"&p=" . $pagina . 
		...
		for ($i = 1; $i <= $pagina; $i++)
		{
			if ($i != $pagina_correta)
			{
				$pag .= "<li><a class='btn btn-dark' href='dashboard.php?tb=filme&op=menu&s="
				. ($exiba * ($i - 1)) . "&p=" .
		...
		{
			$pag .= "<li>
			<a class='btn btn-dark' href='dashboard.php?tb=filme&op=menu&s="
			. ($inicio + $exiba) .
